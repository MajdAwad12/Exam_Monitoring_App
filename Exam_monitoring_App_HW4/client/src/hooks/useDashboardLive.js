...
export function useDashboardLive({ examId = null, roomId, pollMs = 10000, lite = true } = {}) {
  ...
  // 🔴 WebSocket: refresh immediately when server notifies "data changed"
  useWebSocketEvent((msg) => {
    if (!msg || typeof msg !== "object") return;
    if (msg.event !== "SYSTEM_DATA_CHANGED") return;

    const msgExam = String(msg.examId || "");
    const currentExam = String(examIdRef.current || "");

    // If client is looking at a specific exam, refresh only when it matches.
    // If not (e.g., lecturer/supervisor dashboard), refresh on any event.
    if (!currentExam || !msgExam || msgExam === currentExam) {
