...
await saveWithRetry(exam);
    broadcastSystemChange({ examId: examId });
...
await saveWithRetry(exam);
    ...
    tr.toSeat = finalSeat;
    await tr.save();
    broadcastSystemChange({ examId: String(tr.examId || tr.exam || "") });
...
export async function cancelTransfer(req, res) {
  try {
    ...
    exam.markModified("events");
await saveWithRetry(exam);
    broadcastSystemChange({ examId: String(tr.examId || tr.exam || (exam && exam._id) || "") });
