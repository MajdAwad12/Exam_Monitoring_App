...
export default function ActiveClassroomsTabs({
  ...
  const items = useMemo(() => {
    ...
    for (const ex of runningExams || []) {
      ...
      const courseName = ex?.courseName || t("dashboard.common.examFallback");
    ...
    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 p-3">
      <div className="flex items-center justify-between gap-3">
        <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">
          {t("dashboard.activeClassrooms.title")}
        </div>
        <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500">
          {t("dashboard.activeClassrooms.subtitle")}
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={[
                ...
                isActive
                  ? "border-indigo-300 bg-white dark:bg-slate-950 ring-2 ring-indigo-200"
                  : "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:border-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40",
              ].join(" ")}
            >
              <div className="flex items-center gap-2">
                ...
                />
                <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">{it.label}</div>
              </div>
              <div className="mt-0.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 line-clamp-1">
                {it.sub}
              </div>
            </button>
          );
