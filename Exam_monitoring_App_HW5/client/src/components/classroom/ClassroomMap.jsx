...
export default function ClassroomMap({
  ...
  async function markPresentById(idText) {
    ...
    const nowISO = new Date().toISOString();
    const patch = { status: "present", arrivedAt: nowISO };

    // ✅ Do NOT force classroom/seat move when activeRoom is empty.
    // If the student already has a classroom/seat, the server will keep it.
    if (String(activeRoom || "").trim()) {
      patch.classroom = activeRoom;
      patch.roomId = activeRoom;
    }

    await patchStatus(id, patch);
    ...
    <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">Classroom (5×5) • Room {activeRoom || "-"}</div>
              <div className="mt-1 text-sm text-slate-600 dark:text-slate-300">Click a seat for actions • Use QR / ID to mark present</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {canCallLecturer && (
              <button
                ...
                disabled={saving}
                className="px-3 py-2 rounded-2xl text-sm font-extrabold border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40 disabled:opacity-60"
              >
                🗣️ Call Lecturer
              </button>
            )}
            ...
            <button
              ...
              className="px-3 py-2 rounded-2xl text-sm font-extrabold border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40"
            >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-56 max-w-[70vw] px-3 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-sm outline-none focus:ring-2 focus:ring-sky-200"
              />
      ...
      <div className="p-5 bg-slate-50 dark:bg-slate-900/40">
        ...
        {studentsInRoom.length > 0 ? (
          <div className="mb-4 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-2">
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">Students in Room {activeRoom}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold">Total: {studentsInRoom.length}</div>
              </div>
                ...
                <div className="ml-auto flex items-center gap-2">
                  {moreCount > 0 ? (
                    <button
                      ...
                      className="text-xs font-extrabold px-3 py-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-900"
                    >
                      +{moreCount} more
                    </button>
                  ) : null}
                  ...
                  {showAllStudents && rosterFiltered.length > maxPreview ? (
                    <button
                      ...
                      className="text-xs font-extrabold px-3 py-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40"
                    >
                      Collapse
                    </button>
                  ) : null}
              ...
              {visibleRoster.map((a) => {
                ...
                return (
                  <button
                    ...
                    className="text-left rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-900"
                    title="Click to fill the input"
                  >
                    <div className="text-[12px] font-extrabold text-slate-900 dark:text-slate-100 truncate max-w-[220px]">{name}</div>
          ...
          <div className="mb-4 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm overflow-hidden">
          <div className="border-b border-slate-200 dark:border-slate-800">
            ...
            <div className="px-4 py-3 flex items-center justify-between">
              <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">Classroom Map</div>

              <button
                type="button"
                onClick={() => setMapGuideOpen((v) => !v)}
                className="px-3 py-1.5 rounded-2xl text-xs font-extrabold border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40"
              >
                💡 Map Guide
              </button>
            </div>
            ...
            {/* Guide (below header) */}
            {mapGuideOpen && (
              <div className="px-4 pb-4 bg-slate-50 dark:bg-slate-900/40">
                <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-5">
                  {/* Title */}
                  <div className="flex items-center gap-2">
                    <span className="px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 text-sm font-extrabold">
                  ...
                  {/* Footer note */}
                  <div className="mt-4 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 font-semibold">Status colors are shown below the map.</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="relative w-full h-[72vh] min-h-[420px] sm:min-h-[520px] md:h-[680px] overflow-auto overscroll-contain bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
                  ...
                  return (
                    <div key={`seat-${key}`} style={{ gridColumn: gridCol, gridRow: r, height: "clamp(64px, 10vw, 86px)", minWidth: 0 }}>
                      {a ? (
                        ...
                        <div
                          className="w-full h-full rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 grid place-items-center shadow-sm"
                          title={`Empty • ${key}`}
                        >
                          <div className="text-sm font-extrabold text-slate-500 dark:text-slate-400 dark:text-slate-500">Empty</div>
                        </div>
                      )}
                    </div>
                  );
            ...
            <div className="absolute left-1/2 bottom-5 -translate-x-1/2">
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm px-4 py-3 w-[min(320px,86vw)]">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold">Supervisor</div>
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500">
                    Room <span className="font-extrabold text-slate-900 dark:text-slate-100">{activeRoom || "-"}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="p-4 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 text-xs">
              ...
              <LegendDot color="bg-slate-50 dark:bg-slate-900/400" label="Absent" />
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <button
      ...
      className={`text-xs font-extrabold px-3 py-1.5 rounded-2xl border ${
        active ? "bg-sky-600 text-white border-sky-600" : "bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40"
      }`}
      type="button"
    >
      {label}
...
function LegendDot({ color, label }) {
  return (
    <div className="flex items-center gap-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-3 py-2">
