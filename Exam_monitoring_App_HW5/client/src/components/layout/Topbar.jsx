...
export default function Topbar({ me, onOpenSidebar }) {
  ...
  return (
    <header className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
      <div
        className="px-3 sm:px-4 md:px-6 lg:px-10 py-3 sm:py-0 min-h-16 sm:min-h-20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3"
        role="banner"
      >
        {/* Mobile: menu button */}
        ...
        <div className={"min-w-0 flex-1 w-full " + (isRtl ? "order-1" : "order-2")}>
          <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-slate-800 dark:text-slate-100 truncate">
            {t("topbar.greeting")},{" "}
            <span className="text-slate-900 dark:text-white">
              {me?.fullName || t("topbar.defaultUser")}
            </span>{" "}
            👋
          </h2>
        </div>
        ...
        {/* Controls */}
        <div className="w-full sm:w-auto flex flex-col xs:flex-row sm:flex-row sm:items-center sm:justify-end gap-2 sm:gap-3 order-3">
          {/* Date (show from md+; on small screens it takes too much space) */}
          <div className="hidden md:block text-right text-sm text-slate-500 dark:text-slate-300">
            <p>
