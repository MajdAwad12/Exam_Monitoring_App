...
function isLecturer(user) {
  return roleOf(user) === "lecturer";
}

// ✅ exam membership checks (relevant staff only)
function isLecturerAssignedToExam(user, exam) {
  if (!user || !exam) return false;
  const uid = String(user?._id || "");
  if (!uid) return false;

  const mainId = String(exam?.lecturer?.id || "");
  if (mainId && mainId === uid) return true;

  const co = Array.isArray(exam?.coLecturers) ? exam.coLecturers : [];
  return co.some((x) => String(x?.id || "") === uid);
}

function isSupervisorAssignedToExam(user, exam) {
  if (!user || !exam) return false;
  const uid = String(user?._id || "");
  if (!uid) return false;

  const sups = Array.isArray(exam?.supervisors) ? exam.supervisors : [];
  return sups.some((x) => String(x?.id || "") === uid);
}

function canModerateTransfers(user, exam) {
  if (!user) return false;
  if (isAdmin(user)) return true;
  const r = roleOf(user);
  if (r === "lecturer") return isLecturerAssignedToExam(user, exam);
  if (r === "supervisor") return isSupervisorAssignedToExam(user, exam);
  return false;
}
...
function canCancelTransfer(user, tr, exam) {
  ...
  // ✅ Lecturer can cancel only if assigned to this exam
  if (r === "lecturer") return Boolean(exam && isLecturerAssignedToExam(user, exam));

  // ✅ requester can cancel
  if (String(tr.requestedBy?.id || "") === String(user._id || "")) return true;

  // ✅ supervisor of FROM room can cancel (only if assigned to this exam)
  if (r === "supervisor" && exam && isSupervisorAssignedToExam(user, exam)) {
...
export async function approveTransfer(req, res) {
  try {
    ...
    // ✅ Only relevant staff for THIS exam can approve/reject
    if (!canModerateTransfers(user, exam)) {
      return res.status(403).json({ message: "Forbidden" });
    }

    // ✅ Supervisors can approve only transfers INTO their assigned room.
    ...
    if (isSupervisor(user)) {
      ...
      if (canonicalRoomId(exam, myRoom) !== targetRoom) {
        return res.status(403).json({ message: "Forbidden: you can approve only transfers into your room" });
      }
    }
...
await saveWithRetry(exam);
    ...
    tr.lastError = "";
    tr.lastErrorAt = null;
    tr.handledBy = { id: user._id, name: user.fullName || user.username, roomId: canonicalRoomId(exam, myRoom || targetRoom || "") };
    tr.toSeat = finalSeat;
    await tr.save();
...
export async function rejectTransfer(req, res) {
  try {
    ...
    // ✅ Supervisors can reject only transfers INTO their assigned room.
    const targetRoom = canonicalRoomId(exam, tr.toClassroom);
    ...
    if (isSupervisor(user)) {
      ...
      if (canonicalRoomId(exam, myRoom) !== targetRoom) {
        return res.status(403).json({ message: "Forbidden: you can reject only transfers into your room" });
      }
    }
    tr.status = "rejected";
    tr.lastError = "";
