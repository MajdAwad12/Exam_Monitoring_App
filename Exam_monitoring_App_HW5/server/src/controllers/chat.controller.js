...
function normalizeLang(v) {
  ...
  if (raw === "he" || raw === "iw" || raw.startsWith("he-")) return "he";
  return "en";
}

function tr(lang, en, he) {
  return lang === "he" ? he : en;
}

function translateDbTextToHebrew(text) {
  const s = String(text || "");
  if (!s) return s;

  // Common deterministic DB answer wrappers
  return s
    .replace(/^Current time \(([^)]+)\):/m, "השעה הנוכחית ($1):")
    .replace(/^Upcoming exams:/m, "מבחנים קרובים:")
    .replace(/^Next exam:/m, "המבחן הבא:")
    .replace(/^Second upcoming exam:/m, "המבחן השני הקרוב:")
    .replace(/^No running exam found\./m, "לא נמצא מבחן פעיל כרגע.")
    .replace(/^Current exam:/m, "המבחן הנוכחי:")
    .replace(/^Not arrived students:/m, "סטודנטים שלא הגיעו:")
    .replace(/^Attendance summary:/m, "סיכום נוכחות:")
    .replace(/^Transfers summary:/m, "סיכום העברות:")
    .replace(/\bstatus:\s*/g, "סטטוס: ")
    .replace(/\broom\b/gi, "כיתה")
    .replace(/\bstudent\b/gi, "סטודנט")
    .replace(/\bstudents\b/gi, "סטודנטים");
}
/**
...
const FAQ_HE = [
  {
    match: (t) => t.includes("התחל") && t.includes("מבחן"),
    answer:
      "כדי להתחיל מבחן:\n1) כנס/י ל-Manage Exams\n2) בחר/י מבחן במצב Scheduled\n3) לחץ/י Start Exam\n4) עבור/י ל-Dashboard כדי לעקוב בזמן אמת.",
  },
  {
    match: (t) => t.includes("סמן") && (t.includes("נוכחות") || t.includes("הגיע")) ,
    answer:
      "כדי לסמן סטודנט כנוכח:\n1) פתח/י את המפה בכיתה\n2) לחץ/י על הכיסא של הסטודנט\n3) בחר/י Mark Present\n* אם הסטודנט כבר במפה — המיקום שלו לא אמור להשתנות.",
  },
  {
    match: (t) => t.includes("העברה") || (t.includes("להעביר") && t.includes("כיתה")),
    answer:
      "כדי להעביר סטודנט לכיתה אחרת:\n1) פתח/י את Seat Actions במפה\n2) בחר/י Transfer\n3) בחר/י יעד (כיתה + כיסא)\n4) אשר/י.\nהמערכת תתעד את הבקשה ותעדכן בדשבורד.",
  },
  {
    match: (t) => t.includes("שירותים") && (t.includes("יותר") || t.includes(">")),
    answer:
      "אפשר לשאול: 'איזה סטודנטים יצאו לשירותים יותר מ-3 פעמים?'.\nהבוט יחזיר רשימה מתוך הנתונים במסד.",
  },
  {
    match: (t) => t.includes("לא הגיע") || t.includes("לא הגיעו"),
    answer:
      "אפשר לשאול: 'אילו סטודנטים לא הגיעו עדיין? תן שמות'.\nהבוט יחזיר רשימה מתוך הנתונים במסד.",
  },
];

function faqAnswer(userText, lang) {
  const t = String(userText || "").toLowerCase().trim();
  if (!t) return null;
  const list = lang === "he" ? FAQ_HE : FAQ;
  for (const f of list) if (f.match(t)) return f.answer;
  return null;
}
...
async function geminiAnswer({ actor, message, lang }) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return { text: tr(lang, "AI is not configured on the server (missing GEMINI_API_KEY).", "ה-AI לא מוגדר בשרת (חסר GEMINI_API_KEY)." ) };
...
STRICT RULES:
- Always answer in the language requested (HEBREW if lang=he, otherwise ENGLISH).
- If the user asks for ANY database facts (numbers, lists, counts, rankings, schedules, attendance, toilets, transfers, incidents, messages, reports), reply EXACTLY:
  ${lang === "he" ? "השאלה הזו דורשת עובדות ממסד הנתונים. אנא שאל/י זאת כשאילתת דשבורד/DB." : "This question requires database facts. Please ask it as a dashboard/DB query."}
...
export async function chatWithAI(req, res) {
  try {
    ...
    const lang = normalizeLang(req.body?.lang || req.headers["x-lang"]);
    const message = String(req.body?.message || "").trim();

    if (!message) {
      res.status(400).json({ message: tr(lang, "message is required", "חובה לשלוח שאלה (message)" ) });
      return;
    }
    ...
    if (gapMs < minGapMs) {
      res.json({ text: tr(lang, "Please wait a moment, then send your message again.", "אנא המתן/י רגע ושלח/י שוב." ) });
      return;
    }
    ...
    if (isDbRequiredQuestion(t)) {
      const ans0 = await dbDirectAnswer(message, actor);
      const ans = lang === "he" ? translateDbTextToHebrew(ans0) : ans0;
      return res.json({ text: ans || tr(lang, "I don't have that information available in the database right now.", "אין לי כרגע את המידע הזה במסד הנתונים." ) });
    }
    // 3) Non-DB question -> Gemini allowed
    const maxPerDay = Number(process.env.GEMINI_MAX_PER_DAY || 50);
    if (mem.geminiUsed >= maxPerDay) {
      res.json({
        text: tr(lang, "AI quota reached for today. I can still answer dashboard/database questions (counts, rooms, attendance, toilet stats, upcoming exams, transfers, etc.).", "מכסת ה-AI להיום הסתיימה. עדיין אפשר לשאול שאלות דשבורד/מסד נתונים (כמות, כיתות, נוכחות, שירותים, מבחנים קרובים, העברות וכו')."),
      });
      return;
    }
    ...
    mem.geminiUsed += 1;
    const out = await geminiAnswer({ actor, message, lang });
    ...
    res.status(500).json({ message: tr(lang, "Chat failed", "שגיאה בצ'אט" ) });
  }
}
