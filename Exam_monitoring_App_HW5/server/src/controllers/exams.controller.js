...
async function makeUniqueUsername(base) {
  ...
  let username = safe || `student.${Date.now()}`;
  for (let i = 0; i < 20; i++) {
    const exists = await User.findOne({ username }).select("_id").lean();
    if (!exists) return username;
    username = `${safe || "student"}${Math.floor(Math.random() * 10000)}`;
  }
  return `${safe || "student"}${Date.now()}`;
}

async function ensureStudentUser({ firstName, lastName, studentId, email } = {}) {
  const sid = String(studentId || "").trim().replace(/\s+/g, "");
  if (!sid) return null;

  let u = await User.findOne({ role: "student", studentId: sid }).select("_id fullName role studentId username email").lean();
  if (u) return u;

  const fullName = `${String(firstName||"").trim()} ${String(lastName||"").trim()}`.trim() || `Student ${sid}`;
  const baseUsername = `${firstName}.${lastName}.${sid.slice(-4)}`;
  const username = await makeUniqueUsername(baseUsername);
  const em = String(email || "").trim() || `${username}@demo.local`;
  const password = `Stu${sid.slice(-4)}!`; // demo-only
  ...
  return { _id: created._id, fullName: created.fullName, role: created.role, studentId: created.studentId, username: created.username, email: created.email };
}
...
export async function getExams(req, res) {
  try {
    ...
    const exams = await Exam.find(filter)
      .select("_id courseId courseName examMode status startAt endAt classrooms lecturer coLecturers supervisors report.summary createdAt updatedAt")
      .sort({ startAt: -1 })
      .lean();
  ...
  try {
    ...
    const result = await updateExamWithRetry(examId, async (exam) => {
      ...
      if (wantsMove) {
        ...
        const currentRoom = normalizeRoomId(att.classroom || att.roomId);
        const currentSeat = normalizeSeat(att.seat);

        // ✅ אם זה אותו חדר ואותו כיסא של הסטודנט עצמו — לא מזיזים אותו
        if (currentRoom === targetRoom && currentSeat && currentSeat === targetSeat) {
          finalSeat = targetSeat;
        } else {
      ...
      /* ============ Extra Time (minutes) ============ */
      if (patch.extraMinutes !== undefined) {
        const val = Number(patch.extraMinutes);
        if (!Number.isFinite(val) || val < 0) {
          const err = new Error("Invalid extraMinutes");
          err.statusCode = 400;
          throw err;
        }
        const nextExtra = Math.min(240, Math.round(val)); // cap at 4 hours
        const prevExtra = Number(att.extraMinutes || 0) || 0;
        if (prevExtra !== nextExtra) {
          att.extraMinutes = nextExtra;
          att.lastStatusAt = now;

          pushStudentTimeline(exam, realStudentKey, {
            at: now,
            kind: "EXTRA_TIME",
            note: `Extra time set to +${nextExtra} minutes`,
            severity: "low",
            classroom: att.classroom || "",
            seat: att.seat || "",
            meta: { by: actor, prevExtra, nextExtra },
          });
          ...
          pushExamTimeline(exam, {
            ...
            roomId: att.classroom || "",
            actor,
            student: { ...sSnap, classroom: att.classroom || "", seat: att.seat || "" },
            details: { prevExtra, nextExtra },
          });
        }

        // ✅ update exam-wide flag so ALL roles can see extension even if their attendance list is filtered
        exam.extraTimeActive = (exam.attendance || []).some((x) => (Number(x.extraMinutes || 0) || 0) > 0);
      }
...
// Find existing student user or create a new one (system-safe).
let { user: studentUser, studentNumber } = await resolveStudentUser(studentIdInput);
if (!studentUser?._id) {
  studentUser = await ensureStudentUser({
    firstName,
    lastName,
    studentId: studentIdInput,
    email: String(body.email || "").trim(),
  });
  studentNumber = String(studentIdInput || "").trim().replace(/\s+/g, "");
}
if (!studentUser?._id) {
  return res.status(500).json({ message: "FAILED_TO_CREATE_STUDENT_USER" });
}
const now = new Date();
    const actor = actorFromReq(req);
