...
function badge(status) {
  ...
  if (s === "not_arrived") return "bg-slate-50 dark:bg-slate-900/40 border-slate-300 text-slate-700 dark:text-slate-200";
  ...
  if (s === "absent") return "bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200";
  return "bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200";
}
...
function ActionBtn({ label, onClick, disabled, tone = "neutral" }) {
  const toneCls =
    tone === "danger"
      ...
      : "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40 text-slate-900 dark:text-slate-100";
...
async function setExtraMinutes(nextExtraMinutes) {
  if (!canEditAttendance || lockedActions) return;

  const val = Math.max(0, Math.min(240, Math.round(Number(nextExtraMinutes) || 0)));

  setLocalErr("");
  setActionBusy(true);
  try {
    await onSetStatus?.(seat.studentId, { extraMinutes: val });
    onClose?.(); // ✅ close only after success
  } catch (e) {
    const msg = e?.message || String(e);
    setLocalErr(msg);
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/40 grid place-items-center p-3 sm:p-4">
      <div className="w-full max-w-[720px] max-h-[85vh] rounded-3xl bg-white dark:bg-slate-950 shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
        {/* header */}
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Student</div>
            <div className="text-lg sm:text-xl font-extrabold text-slate-900 dark:text-slate-100 truncate">
              {seat.name || "Student"}{" "}
              <span className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold text-sm">
                ({seat.studentNumber || seat.studentId || "-"})
              </span>
            </div>

            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
              <span className="px-3 py-1 rounded-full border bg-slate-50 dark:bg-slate-900/40 text-slate-700 dark:text-slate-200">
                Room <b>{roomNow || "-"}</b>
              </span>
              <span className="px-3 py-1 rounded-full border bg-slate-50 dark:bg-slate-900/40 text-slate-700 dark:text-slate-200">
                Seat <b>{seatLabel}</b>
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-900 dark:bg-slate-900/40 font-extrabold shrink-0"
          >
            Close
          </button>
        </div>
        ...
        <div className="p-4 sm:p-5 space-y-4 overflow-y-auto">
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 p-4">
            <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">Quick actions</div>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {/* Extra time */}
                  <div className="mt-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-950/40 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-xs font-extrabold text-slate-900 dark:text-slate-100">
                        {t("seatActions.extraTime.title", "Extra time")}
                      </div>
                      <div className="text-xs font-extrabold text-slate-700 dark:text-slate-200">
                        {t("seatActions.extraTime.current", "Current")}:{" "}
                        <span className="text-sky-700 dark:text-sky-300">+{Number(seat?.extraMinutes || 0) || 0}m</span>
                      </div>
                    </div>

                    <div className="mt-2 grid grid-cols-2 sm:grid-cols-5 gap-2">
                      <ActionBtn
                        disabled={!canEditAttendance || lockedActions}
                        onClick={() => setExtraMinutes((Number(seat?.extraMinutes || 0) || 0) + 15)}
                        label={t("seatActions.extraTime.add15", "➕ 15m")}
                        tone="purple"
                      />
                      <ActionBtn
                        disabled={!canEditAttendance || lockedActions}
                        onClick={() => setExtraMinutes((Number(seat?.extraMinutes || 0) || 0) + 30)}
                        label={t("seatActions.extraTime.add30", "➕ 30m")}
                        tone="purple"
                      />
                      <ActionBtn
                        disabled={!canEditAttendance || lockedActions}
                        onClick={() => setExtraMinutes((Number(seat?.extraMinutes || 0) || 0) + 45)}
                        label={t("seatActions.extraTime.add45", "➕ 45m")}
                        tone="purple"
                      />
                      <ActionBtn
                        disabled={!canEditAttendance || lockedActions}
                        onClick={() => setExtraMinutes(0)}
                        label={t("seatActions.extraTime.clear", "✖ Clear")}
                        tone="danger"
                      />
                    ...
                    <div className="mt-2 text-[11px] text-slate-600 dark:text-slate-300">
                      {t(
                        "seatActions.extraTime.hint",
                        "When extra time is active, the whole exam end time extends automatically (15 minutes per hour)."
                      )}
                    </div>
                  </div>
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">Transfer student</div>
            ...
            <div className="mt-3 flex flex-col sm:flex-row gap-2">
              <select
                ...
                className="w-full sm:w-[260px] px-3 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-sm font-bold outline-none focus:ring-2 focus:ring-purple-200 disabled:opacity-60"
              >
            ...
            {!canRequestTransfer ? (
              <div className="mt-2 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Only Lecturer/Supervisor can request transfers.</div>
            ) : isTransferPending ? (
              <div className="mt-2 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">
                Transfer is pending. You can cancel it (and actions will unlock).
              </div>
            ) : lockedActions ? (
              <div className="mt-2 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Student is currently moving.</div>
            ) : null}
          </div>

          {/* Incident note */}
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-4">
            <div className="text-sm font-extrabold text-slate-900 dark:text-slate-100">Incident / Cheat note</div>
            <div className="mt-2 text-xs text-slate-600 dark:text-slate-300">Saved to report timeline and student file.</div>
            ...
            <textarea
              ...
              className="mt-3 w-full px-3 py-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-sm outline-none focus:ring-2 focus:ring-rose-200 disabled:opacity-60"
              maxLength={400}
            />

            <div className="mt-3 flex items-center justify-between gap-2">
              <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{note.length}/400</div>
              <button
                onClick={submitNote}
